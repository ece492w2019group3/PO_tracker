...
static  void  AppTaskStart (void *p_arg)
{
    ...
    test_target_device();
    test_master_send(0x0f, 1);
	test_master_receive(1);
...
//	test[2] = 0;
//	test[3] = 0;
	int test = CTRL_REG1_G;
	test += 0xDB << 8;

//	printf("test:%x", test);

	test_master_send(test, 2);
    ...
    for(;;) {
        ...
        test_master_send(0x18, 1);
        test_master_receive(2);

        test_master_receive(2);

        test_master_receive(2);
        printf("accelerometer readings\n");
        test_master_send(0x28, 1);
		test_master_receive(2);
